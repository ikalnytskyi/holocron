...
def test_item(testapp):
    ...
    stream = archive.process(
        testapp,
        [core.Item({"title": "the way of the Force", "content": "Obi-Wan"})],
    )

    assert next(stream) == core.Item(
        {"title": "the way of the Force", "content": "Obi-Wan"}
    )
...
@pytest.mark.parametrize(
    "save_as",
    [os.path.join("posts", "skywalker.luke"), os.path.join("yoda.jedi")],
)
